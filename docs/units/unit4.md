...
**הערה 10.11.** מתקיים גם \(\int_{g^{-1}(a)}^{g^{-1}(b)} f(g(x)) |g'(x)|\,dx = \int_a^b f(x)\,dx\) (תלוי בסדר הגבולות).

---

### דוגמאות מהסיכומים

**דוגמה — \(\int_0^1 \sqrt{1-x^2}\,dx\).**  
נגדיר \(g(x) = \sin x\) על \([0,\frac{\pi}{2}]\). אז \(g^{-1}(0)=0\), \(g^{-1}(1)=\frac{\pi}{2}\). מתקבל
\[
\int_0^1 \sqrt{1-x^2}\,dx = \int_0^{\pi/2} \sqrt{1-\sin^2 x} \cdot \cos x\,dx = \int_0^{\pi/2} \cos^2 x\,dx = \int_0^{\pi/2} \frac{1+\cos(2x)}{2}\,dx.
\]
קדומה: \(\frac{x}{2} + \frac{\sin(2x)}{4}\). לפי ניוטון־לייבניץ: \(\frac{\pi}{4}\).

**דוגמה — טעות נפוצה.** בחישוב \(\int_{-1}^1 \frac{1}{1+x^2}\,dx\) עם שינוי משתנה \(g(x)=1/x\) — \(g\) **לא מוגדרת** ב־0, והתחום/טווח לא תואמים (למשל \(g((0,1]) = [1,+\infty)\)). אין להפעיל את משפט שינוי המשתנה; האינטגרל מחשבים ישירות: \(\arctan(1)-\arctan(-1) = \frac{\pi}{2}\).
...
**פתרון:** נגדיר \(f(x) = \frac{1}{x}\), \(g(x) = \sin x\) על \([2\pi, 3\pi]\). \(f,g\) אלמנטריות ואינטגרביליות; \(g \ge 0\). לפי טענה 10.13 קיים \(c \in [2\pi, 3\pi]\) כך ש־\(\int_{2\pi}^{3\pi} \frac{\sin x}{x}\,dx = \frac{1}{c} \int_{2\pi}^{3\pi} \sin x\,dx = \frac{2}{c}\). מתקיים \(\int_{2\pi}^{3\pi} \sin x\,dx = [-\cos x]_{2\pi}^{3\pi} = -(-1) - (-1) = 2\). כיוון ש־\(c \in [2\pi, 3\pi]\), מתקיים \(\frac{1}{3\pi} \le \frac{1}{c} \le \frac{1}{2\pi}\), ומכאן \(\frac{2}{3\pi} \le \int_{2\pi}^{3\pi} \frac{\sin x}{x}\,dx \le \frac{2}{2\pi} = \frac{1}{\pi}\).

---

## דוגמאות נוספות לשינוי משתנה באינטגרל מסוים

### דוגמה — חישוב \(\int_0^1 \sqrt{1-x^2}\,dx\)
...
## נספח שיעור 24 — תרגילים נוספים

1. חשבו את האינטגרלים הבאים:
   - \(\int_1^e (\ln x)^2\,dx\)
   - \(\int_0^{\pi/4} \tan^2 x\,dx\)
   - \(\int_0^1 x \arctan x\,dx\)

2. הוכיחו את נוסחת הרדוקציה: \(\int_0^{\pi/2} \sin^n x\,dx = \frac{n-1}{n} \int_0^{\pi/2} \sin^{n-2} x\,dx\).

3. חשבו \(\int_0^1 \frac{\ln(1+x)}{x}\,dx\) (רמז: פתחו את \(\ln(1+x)\) לטור).

4. תהי \(f : [a,b] \to \mathbb{R}\) רציפה. הוכיחו כי:
   \[
